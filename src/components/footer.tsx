import Link from "next/link";

export function Footer() {
  return (
    <footer className="bg-white border-t border-[#E0E0E0]">
      {/* 상단: 법적 링크 + 로고 */}
      <div className="max-w-[1480px] mx-auto px-4 lg:px-10 pt-10 pb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          {/* 로고/회사명 */}
          <span className="text-[1rem] font-bold text-[#1A1A1A]">
            정율 교육정보
          </span>

          {/* 법적 링크 — 삼성 뉴스룸 상단 우측 스타일 */}
          <div className="flex flex-wrap items-center gap-5 text-[0.875rem] font-bold text-[#1A1A1A]">
            <Link href="/faq" className="hover:text-[#1E64FA] transition-colors">
              자주 묻는 질문
            </Link>
            <Link href="/about" className="hover:text-[#1E64FA] transition-colors">
              회사소개
            </Link>
            <Link href="/privacy" className="hover:text-[#1E64FA] transition-colors">
              개인정보처리방침
            </Link>
            <Link href="/terms" className="hover:text-[#1E64FA] transition-colors">
              이용약관
            </Link>
          </div>
        </div>
      </div>

      {/* 중단: 회사정보 + 저작권 */}
      <div className="max-w-[1480px] mx-auto px-4 lg:px-10 pb-6">
        <div className="text-[0.875rem] text-[#666666] leading-relaxed space-y-1">
          <p>주식회사정율 | 대표이사: 곽정율</p>
          <p>경기도 부천시 원미구 길주로91 601호(비잔티움 6층)</p>
          <p>
            사업자등록번호: 392-88-00208
            <span className="inline-block w-px h-3 bg-[#E0E0E0] mx-2 align-middle" />
            학원등록번호: 제5042호
          </p>
          <p>
            고객센터: 032-321-9937
            <span className="inline-block w-px h-3 bg-[#E0E0E0] mx-2 align-middle" />
            이메일: jungyoul3@naver.com
          </p>
        </div>
      </div>

      {/* 하단: SNS + 저작권 */}
      <div className="max-w-[1480px] mx-auto px-4 lg:px-10 pb-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pt-6 border-t border-[#E0E0E0]">
          {/* SNS 아이콘 — 삼성 뉴스룸 라이트 스타일 */}
          <div className="flex items-center gap-3">
            <a
              href="https://www.instagram.com/jysk_official/"
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 bg-[#F5F5F5] rounded-full flex items-center justify-center hover:bg-[#e9e9e9] transition-colors"
              aria-label="Instagram"
            >
              <svg className="w-5 h-5 text-[#1A1A1A]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>
            <a
              href="https://blog.naver.com/jungyoul_edu"
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 bg-[#F5F5F5] rounded-full flex items-center justify-center hover:bg-[#e9e9e9] transition-colors"
              aria-label="네이버 블로그"
            >
              <span className="text-[#1A1A1A] text-xs font-bold">N</span>
            </a>
            <a
              href="http://pf.kakao.com/_xiqxhxlxb"
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 bg-[#F5F5F5] rounded-full flex items-center justify-center hover:bg-[#e9e9e9] transition-colors"
              aria-label="카카오톡"
            >
              <span className="text-[#1A1A1A] text-xs font-bold">K</span>
            </a>
            <a
              href="https://www.youtube.com/@jungyoulTV"
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 bg-[#F5F5F5] rounded-full flex items-center justify-center hover:bg-[#e9e9e9] transition-colors"
              aria-label="YouTube"
            >
              <svg className="w-5 h-5 text-[#1A1A1A]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
              </svg>
            </a>
          </div>

          {/* 저작권 */}
          <p className="text-[0.875rem] text-[#666666]">
            Copyright &copy; {new Date().getFullYear()} Jungyoul Co., Ltd. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
  );
}
