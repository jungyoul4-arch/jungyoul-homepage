...
1. **HTML 마크업 (최우선)**: text/html 에 `<table|img|p|h1-6|ul|ol|figure|blockquote>` 중 하나라도 있으면 `normalizePastedHtml()` 통과 후 삽입. data:URL 이미지는 R2 로 업로드되어 영구 URL 로 치환.
2. **단독 이미지**: `clipboardData.items` 에 `image/*` 가 있으면 (스크린샷 캡쳐 등) `handleImageFiles()` → R2 업로드 → `<figure><img><figcaption>`
