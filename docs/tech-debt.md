...
### 약점
- 테스트 커버리지 **0%**, CI 검증 단계 부재
- DB 인덱스 누락, FK 제약 전무
- R2 incremental cache 비활성 (ISR 캐시 휘발)
- 에러 처리 산발적 — 침묵 catch, 제네릭 500
...
| 항목 | 근거 |
|---|---|
| 단위/통합/E2E 테스트 0건 | API 라우트 55개, 컴포넌트 29개, `src/lib` 15개 전부 미검증 |
| 스모크 인프라 없음 | `scripts/` 디렉터리·`smoke*.mjs`·`*.test.ts` 모두 없음 |
| 테스트 셋업 롤백 이력 | `c583e7a` 에서 vitest + `src/lib/__tests__/*.test.ts` 8종 삭제 |
| CI 검증 부재 | `.github/workflows/deploy.yml` 에 lint/type-check/test 단계 없이 main push 시 즉시 프로덕션 배포 |
| pre-commit 제한적 | `.husky/pre-commit` 은 lint-staged(eslint --fix + tsc --noEmit)만, 빌드/테스트 없음 |

### B. 데이터베이스 — 높음

- **인덱스 누락** (`src/db/schema.ts`): `articles.category`, `articles.date`, `communityPosts.sessionId`, `communityPosts.isDeleted`, `communityComments.postId` — /exam 필터·기사 목록 정렬·커뮤니티 피드의 빈번한 조회 경로. 데이터 증가 시 풀스캔.
...
- **대형 파일** (>400줄): `src/components/content-editor.tsx`(924), `src/app/admin/slides/page.tsx`(681), `src/components/thumbnail-overlay-editor.tsx`(679), `src/components/hero-carousel.tsx`(607) 외 4개.
...
- **Miniflare D1 에러 헬퍼 미적용**: 공용 헬퍼 `isUniqueConstraintError()` 가 `src/lib/validation.ts` 에 **이미 존재**하나, 일부 생성(POST) 라우트가 이를 쓰지 않아 UNIQUE 위반이 409 대신 500 으로 반환됨(정정 — 신규 헬퍼 생성 불필요).

### D. 에러 처리 — 중간

- **침묵 catch** (`.catch(() => {})` — 실패가 사용자/로그에 노출 안 됨):
  - `src/components/community/community-feed.tsx:41`
  - `src/components/community/like-button.tsx:29`
  - `src/app/admin/settings/page.tsx:18`
- **제네릭 500**: API 라우트 다수가 `errorResponse` 로 검증 에러와 DB 에러를 구분 없이 500 처리 — 로깅·구조화 부재, 프로덕션 스택트레이스 유실.

### E. 배포 / 캐싱 — 중간

- `open-next.config.ts` 의 R2 incremental cache 가 주석 처리됨 (OpenNext 기본 스캐폴드 그대로). ISR 캐시가 휘발성 인메모리로 폴백 → 서버리스에서 캐시 신뢰성 낮음.
- CI 에 프리뷰/스테이징 환경 없음 — main push = 즉시 프로덕션. D1 마이그레이션 카나리 절차 없음.
...
- `docs/mistake-log.md` 에 22건 회귀 기록. sanitize 화이트리스트 2단계 동기화 누락, hex 토큰 마이그레이션 누락, 페이스트 분기 우선순위 등 **"수동 규율 의존"** 패턴이 반복적으로 회귀를 유발.
...
| 우선순위 | 항목 | 영역 | 예상 공수 | 근거 |
|---|---|---|---|---|
| **P0** | DB 인덱스 5종 추가 (마이그레이션) | B | 2~3h | 데이터 증가 시 성능 급락, 저비용 고효율 |
| **P0** | 마이그레이션 `0006` 끝줄 개행 정정 | B | 5m | 파일 위생 (빈 파일 아님 — 정정) |
| **P0** | R2 incremental cache 활성화 | E | 0.5h | ISR 안정성, 한 줄 수정 |
| **P1** | 테스트 프레임워크 재도입 (소규모·점진) — `src/lib` 단위테스트부터(sanitize·json-ld·mappers·validation). `c583e7a` 교훈대로 작은 PR + 배포 검증 | A | 3~5d | 안전망 부재가 최대 리스크 |
| **P1** | CI 에 lint + type-check + build 단계 추가 | A,E | 0.5d | 설정 표류·빌드 깨짐 사전 차단 |
