...
npm run test:coverage              # 커버리지 리포트 (lcov + 텍스트)
npm run build                      # Next.js production 빌드
graphify update .                  # 코드 그래프 갱신 (코드 변경 후 필수)
```

> 테스트 대상은 `src/lib/__tests__/**.test.ts`. 커버리지 범위는 `vitest.config.ts` 의 `coverage.include`/`exclude` 참조 — `auth/admin-auth/anon-session/validation` 은 next/server·DB 의존이 무거워 커버리지 제외.
